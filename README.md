...
I found insufficient evidence that transitioning in under three months was associated with improved retention. However, members with transition times of 12 months or more had significantly lower retention than members with shorter transition times. I interpreted this finding with caution given limitations, which are discussed in [Limitations](#Limitations).

## Data

### Data Description

The analysis uses **member-month** data, where each row represents a member and month they were enrolled in the program. The synthetic dataset contains 3,000 members across roughly 115,000 member-month records.
...
Outcome: Time housed - the number of days from a member's lease start date until separation, or if they didn't separate, the last day of the data extract. This is their observed period.
